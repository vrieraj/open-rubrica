"""
utils.py — Utilidades de firma digital y renderizado de PDF
  · Listar certificados del DNIe (PKCS#11)
  · Crear signers pyhanko (DNIe o .p12)
  · Ejecutar firma incremental en múltiples páginas
  · Renderizar páginas PDF como PNG (pymupdf)
"""
...
def listar_certificados(lib_path=DEFAULT_PKCS11_LIB):
    """
    Devuelve lista de dicts con los certificados disponibles en el token.
...
# ─────────────────────────────────────────────────────────────────────────────
# RENDERIZADO DE PÁGINAS (pymupdf → PNG base64)
# ─────────────────────────────────────────────────────────────────────────────

def renderizar_pagina(pdf_bytes: bytes, numero: int, dpi: int = 150) -> bytes:
    """
    Renderiza la página `numero` (1-based) del PDF y devuelve PNG en bytes.
    """
    import fitz  # pymupdf
...
def info_paginas(pdf_bytes: bytes):
    """Devuelve (total_paginas, [(w_pts, h_pts), ...]) para cada página."""
    import fitz
    doc  = fitz.open(stream=pdf_bytes, filetype='pdf')
...
def _construir_sello(texto_firma: str, font_size: int, rubrica_b64: str | None):
    """Crea el TextStampStyle con o sin rúbrica."""
    ...
    if rubrica_b64:
        ...
        raw     = base64.b64decode(rubrica_b64.split(',', 1)[1])
...
def _firmar_pagina(writer, signer, nombre_campo, sello):
    """Firma síncrona usando un event loop + executor de 1 hilo (thread-safe PKCS#11)."""
...
def firmar_pdf(
    pdf_bytes:   bytes,
    output_path: str,
    coords:      dict,          # {num_pagina: (x1,y1,x2,y2)}  coordenadas PDF pts
    texto_firma: str,
    font_size:   int   = 9,
    rubrica_b64: str | None = None,
    # DNIe
    dnie:        bool  = False,
    ...
    if dnie:
        ...
        if not slots_:
            raise RuntimeError("DNIe no detectado.")
    ...
    sello = _construir_sello(texto_firma, font_size, rubrica_b64)
    ...
    try:
        for num_pagina in sorted(coords.keys()):
            ...
            # Guardar y usar como base para la siguiente firma
            datos = out_buf.read()
...
def nombre_salida(input_path: str) -> str:
    """Genera la ruta de salida: /ruta/archivo_firmado.pdf"""
